...
class SyndExport
{
	...
	*/
	...
	private function extractAtomItems($max,$other=NULL)//Extracts ATOM items
	{
		...
		{
			$items[$i] = new Items();
			if(!empty($this->p_feed->entry[$i]->title))		      $items[$i]->setTitle((string)$this->p_feed->entry[$i]->title);
			if(!empty($this->p_feed->entry[$i]->summary)) 	  $items[$i]->setDescription((string)$this->p_feed->entry[$i]->summary);
			elseif(!empty($this->p_feed->entry[$i]->content)) $items[$i]->setDescription((string)$this->p_feed->entry[$i]->content);
			if(!empty($this->p_feed->entry[$i]->link[href]))      $items[$i]->setUrl((string)$this->p_feed->entry[$i]->link[href]);
			if(!empty($this->p_feed->entry[$i]->author)) 	      $items[$i]->setAuthor((string)$this->p_feed->entry[$i]->author);
			if(!empty($this->p_feed->entry[$i]->updated)) 	  $items[$i]->setDate(strtotime((string)$this->p_feed->entry[$i]->updated));
			if(!empty($this->p_feed->entry[$i]->id))		          $items[$i]->setGuid((string)$this->p_feed->entry[$i]->id);
			/****Enclosure****/
			/*
			$j = $i+1;
	...
	private function extractRssItems($max,$other=NULL)//Extracts RSS items
	{
		...
		{
			$items[$i] = new Items();
			if(!empty($this->p_feed->channel->item[$i]->title))		    $items[$i]->setTitle((string)$this->p_feed->channel->item[$i]->title);
			if(!empty($this->p_feed->channel->item[$i]->description))  $items[$i]->setDescription((string)$this->p_feed->channel->item[$i]->description);
			elseif(isset($ns['content']))if(!empty($content_encoded)) $items[$i]->setDescription((string)$this->p_feed->channel->item[$i]->children($ns['content']));//namespace content
			if(!empty($this->p_feed->channel->item[$i]->link))		    $items[$i]->setUrl((string)$this->p_feed->channel->item[$i]->link);
			if(!empty($this->p_feed->channel->item[$i]->author)) 	    $items[$i]->setAuthor((string)$this->p_feed->channel->item[$i]->author);
			if(!empty($this->p_feed->channel->item[$i]->pubDate)) 	$items[$i]->setDate(strtotime((string)$this->p_feed->channel->item[$i]->pubDate));
			if(!empty($this->p_feed->channel->item[$i]->guid))		    $items[$i]->setGuid((string)$this->p_feed->channel->item[$i]->guid);
			/****Start Enclosure****/
			/*
			...
			*/
			/****End Enclosure****/
			//if(!empty($this->p_feed->channel->item[$i]->$other))	$items[$i]["other"] = (string)$this->p_feed->channel->item[$i]->$other;
		}
		return $items;
	}
	...
	public function modifyDescriptionOfItem($itemNumber,$value) {
		if(!empty($value) && $itemNumber >= 0 && $itemNumber < $this->p_count) {
			if($this->p_type == "RSS") {
				if(!empty($this->p_feed->channel->item[$itemNumber]->description)) $this->p_feed->channel->item[$itemNumber]->description = $value;
				elseif(isset($ns['content'])) {
					$dom=dom_import_simplexml($this->p_feed->channel->item[$i]->children($ns['content']));
					$dom->parentNode->removeChild($dom);
					
					$this->p_feed->channel->item[$i].addChild('content',$value,$ns['content']);
					
				}
			} elseif($this->p_type == "ATOM") {
				if(!empty($this->p_feed->entry[$i]->summary)) 	 $this->p_feed->entry[$i]->summary = $value;
				elseif(!empty($this->p_feed->entry[$i]->content)) $this->p_feed->entry[$i]->content = $value;
			}
		}
	}

	public function modifyFieldOfItem($itemNumber,$fieldName,$value) {
		if(!empty($value) && !empty($fieldName) && $itemNumber >= 0 && $itemNumber < $this->p_count) {
			if($this->p_type == "RSS") {
				if(!empty($this->p_feed->channel->item[$itemNumber]->$fieldName))	$this->p_feed->channel->item[$itemNumber]->$fieldName = $value;
			} elseif($this->p_type == "ATOM") {
				if(!empty($this->p_feed->entry[$iitemNumber]))	$this->p_feed->entry[$iitemNumber]->$fieldName = $value;
			}
		}
	}
	
	public function modifyTitleOfFlow($value) {
		if(!empty($value)) {
			if($this->p_type == "RSS") {
				if(!empty($this->p_feed->channel->title)) $this->p_feed->channel->title = $value;
			} elseif($this->p_type == "ATOM") {
				if(!empty($this->p_feed->title)) $this->p_feed->title = $value;
			}
		}
	}
	
	public function changeEncoding($enc) {
		if(!empty($this->p_feed)) {
			$this->p_feed =  new SimpleXMLElement("<?xml version='1.0' encoding='".$enc."'?>".$this->p_feed->asXml());
			$this->p_feed->addAttribute('encoding',$value);
			return true;
		}
		return false;
	}
